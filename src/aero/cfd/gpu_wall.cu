...
__global__ void __launch_bounds__(128) wall_force_kernel(
    ...
    __shared__ Real s_forces[7];

    for (int i = threadIdx.x; i < 7; i += blockDim.x) s_forces[i] = 0.0f;
    __syncthreads();
    ...
    for (int i = 0; i < 7; ++i) real_atomic_add(&s_forces[i], local_f[i]);
    __syncthreads();

    if (threadIdx.x == 0) {
        for (int i = 0; i < 7; ++i) real_atomic_add(&d_forces[i], s_forces[i]);
    }
}
